...
<html lang="en">
   <head>
      ...
      <title>The Album Page</title>
      <!-- Bootstrap -->
      <link href="css/bootstrap.min.css" rel="stylesheet">
      <link href="css/mystylesheetAlbum.css" rel = "stylesheet">
   ...
   <body>
      <div class = 'container'>
         ...
         <form action="count_tuples.php">
            <div id = 'count'>
               <button type="submit" class="btn btn-default">Count Tuples</button>
            </div>
         </form>
         ...
         <form action = "album.php" method="POST" class="form-inline" role="form">
      ...
      <input type="date" class="form-control" action="album.php" method="POST" name="start" id = "StartDate" 		placeholder="Start Year, ex.1930">
      <label for="exampleInputFile">End Year</label>
      <input type="date" class="form-control" action="album.php"method="POST" name="end" id = "EndDate" 			placeholder="End Year, ex.1990">
      </div>
      </form>
      ...
      <div style = "position = absolute" id = "position">
         <?php
            if($bar != NULL && $start == NULL && $end == NULL) {
            	...
            	$statement = oci_parse($connection, "SELECT ALBUM_NAME as Album, ALBUM_YEAR as Year, ARTIST_NAME as Artist, SONG_TITLE AS Song FROM ALBUM, ARTIST, SONG WHERE
            	ALBUM.ARTIST_ID = ARTIST.ARTIST_ID AND ALBUM.SONG_ID = SONG.SONG_ID AND UPPER(ALBUM_NAME) LIKE UPPER('%" . $bar . "%') AND ALBUM_YEAR != 0 AND rownum <= 100 order by ALBUM_NAME,
            	ALBUM_YEAR, ARTIST_NAME, SONG_TITLE asc");
            ...
            elseif($bar == NULL && $start != NULL && $end != NULL) {
            	...
            	$statement = oci_parse($connection, "SELECT ALBUM_NAME AS Album, ALBUM_YEAR AS Year, ARTIST_NAME as Artist, SONG_TITLE as Song
            	FROM ARTIST, SONG, ALBUM WHERE ALBUM.ARTIST_ID = ARTIST.ARTIST_ID AND ALBUM.ARTIST_ID = SONG.ARTIST_ID
            	AND ALBUM.ALBUM_YEAR BETWEEN '" . $start . " 'AND '" . $end . "' AND rownum <= 100 ORDER BY ALBUM_YEAR DESC");
            	oci_execute($statement);
            ...
            elseif($bar != NULL && $start != NULL && $end != NULL) {
            	...
            	$statement = oci_parse($connection, "SELECT ALBUM_NAME AS Album, ALBUM_YEAR AS Year, ARTIST_NAME as Artist, SONG_TITLE as Song
            	FROM ARTIST, SONG, ALBUM WHERE ALBUM.ARTIST_ID = ARTIST.ARTIST_ID AND ALBUM.ARTIST_ID = SONG.ARTIST_ID AND UPPER(ALBUM_NAME) LIKE UPPER('%" . $bar . "%')
            	AND ALBUM.ALBUM_YEAR BETWEEN '" . $start . " 'AND '" . $end . "' AND rownum <= 100 ORDER BY ALBUM_YEAR DESC");
            	oci_execute($statement);
            ...
            elseif($search != NULL) {
            	...
            	$statement = oci_parse($connection, "SELECT ALBUM_NAME as Album, ALBUM_YEAR as Year, ARTIST_NAME as Artist, SONG_TITLE AS Song FROM ALBUM, ARTIST, SONG WHERE
            	ALBUM.ARTIST_ID = ARTIST.ARTIST_ID AND ALBUM.SONG_ID = SONG.SONG_ID AND UPPER(ALBUM_NAME) LIKE UPPER('%" . $search . "%') AND ALBUM_YEAR != 0 AND rownum <= 100 order by ALBUM_NAME,
            	ALBUM_YEAR, ARTIST_NAME, SONG_TITLE asc");
            	oci_execute($statement);
